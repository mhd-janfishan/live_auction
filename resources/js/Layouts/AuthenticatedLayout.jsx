...
export default function AuthenticatedLayout({ header, children }) {
    ...
    const adminNavigation = [
        { name: 'Admin Dashboard', href: route('admin.dashboard'), icon: (
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
            </svg>
        )},
        { name: 'Manage Auctions', href: '#', icon: (
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    const navigation = user.roles.includes('admin') ? adminNavigation : userNavigation;
            ...
            <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out`}>
                <div className="flex h-16 items-center justify-between px-4 border-b border-slate-200">
                    <Link href="/" className="flex items-center">
                        <span className="text-xl font-semibold text-slate-800">LiveBid</span>
                    </Link>
                    <button
                ...
                <div className="px-4 py-3 border-b border-slate-200">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        user.roles.includes('admin') ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800'
                    }`}>
            ...
            <div className={`${sidebarOpen ? 'lg:pl-64' : ''} flex flex-col min-h-screen transition-all duration-300`}>
                        ...
                        <div className="flex h-16 justify-between">
                            <div className="flex items-center">
                                <button
                                    ...
                                    className={`${sidebarOpen ? 'lg:hidden' : ''} -ml-2 mr-2 p-2 rounded-md text-slate-400 hover:bg-slate-100 hover:text-slate-500`}
                                >
                                    <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div className="hidden sm:ms-6 sm:flex sm:items-center">
                                <div className="relative ms-3">
                                    <Dropdown>
                                        ...
                                        <Dropdown.Content>
                                            <Dropdown.Link href={route('profile.edit')} className="block w-full px-4 py-2 text-left text-sm leading-5 text-slate-700 hover:bg-blue-50 focus:bg-blue-50">
                                                Profile
                                            </Dropdown.Link>
                                            <Dropdown.Link href={route('logout')} method="post" as="button" className="block w-full px-4 py-2 text-left text-sm leading-5 text-slate-700 hover:bg-blue-50 focus:bg-blue-50">
                                                Log Out
                                            </Dropdown.Link>
                                        </Dropdown.Content>
                                    </Dropdown>
                    ...
                    <div className={(showingNavigationDropdown ? 'block' : 'hidden') + ' sm:hidden'}>
                        ...
                        <div className="border-t border-slate-200 pb-1 pt-4">
                            ...
                            <div className="mt-3 space-y-1">
                                <ResponsiveNavLink href={route('profile.edit')} className="block py-2 ps-3 pe-4 text-base font-medium text-slate-600 hover:bg-blue-50 hover:text-blue-500">
                                    Profile
                                </ResponsiveNavLink>
                                <ResponsiveNavLink method="post" href={route('logout')} as="button" className="block w-full py-2 ps-3 pe-4 text-left text-base font-medium text-slate-600 hover:bg-blue-50 hover:text-blue-500">
                                    Log Out
                                </ResponsiveNavLink>
                            </div>
                        </div>
                ...
                {header && (
                    <header className="bg-white shadow">
                        <div className="mx-auto px-4 py-6 sm:px-6 lg:px-8">{header}</div>
                    </header>
                )}
